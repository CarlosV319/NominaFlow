...
const QuotaIndicator = () => {
    ...
    return (
        <div className="mx-2 mb-2 p-2 bg-slate-50 border border-slate-200 rounded-lg shadow-sm">
            <div className="flex items-center justify-between mb-2">
                <h3 className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">Tu Plan</h3>
                <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${isFree ? 'bg-slate-200 text-slate-600' : 'bg-[#0F2C4C] text-white'}`}>
                    {plan.name}
                </span>
            </div>
            ...
            {isFree && (
                <button
                    ...
                    className="w-full mt-1.5 flex items-center justify-center gap-1.5 bg-gradient-to-r from-amber-500 to-orange-600 text-white text-[10px] font-bold py-1.5 rounded hover:from-amber-600 hover:to-orange-700 transition"
                >
                    <Zap size={12} fill="currentColor" /> Actualizar a PRO
                </button>
            )}
